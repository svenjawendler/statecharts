...
public class CoreLibraryDefaultFeatureValueProvider extends AbstractDefaultFeatureValueProvider {
	...
	public IStatus validateParameterValue(FeatureParameterValue parameterValue) {
		String parameterName = parameterValue.getParameter().getName();
		if (OUTLET_FEATURE_TARGET_PROJECT.equals(parameterName) && !exists(parameterValue.getStringValue()))
			return error(String.format("The Project %s does not exist or is not open.", parameterValue.getExpression()));
		//TODO the check if a project is open is EFS specific, so exists might be sufficient - opened might be an implementation detail of e.g. the EFS based FSA
		//		if (OUTLET_FEATURE_TARGET_PROJECT.equals(parameterName) && exists(parameterValue.getStringValue()))
//			return error(String.format("The Project %s is not open.", parameterValue.getExpression()));
		if (OUTLET_FEATURE_TARGET_FOLDER.equals(parameterName)
			...
			if (targetProjectName != null && !exists(targetProjectName + "/" + parameterValue.getStringValue())) {
